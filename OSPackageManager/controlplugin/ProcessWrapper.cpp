...
std::vector<pid_t> ProcessWrapper::getRunningProcesses()
{
    ...
    int processCount = proc_listpids(PROC_ALL_PIDS, 0, nullptr, 0);
    if (processCount == 0)
    {
        CM_LOG_ERROR("There are no processes running.");
        return ret;
    }
    else if (processCount < 0)
    {
        CM_LOG_ERROR("Unablee to determine amount of running processes.");
        return ret;
    }
    
    size_t nBufSize = processCount * sizeof(pid_t);
    if (nBufSize > static_cast<size_t>(std::numeric_limits<int>::max()))
    {
        CM_LOG_ERROR("Size or processes buffer exceeds max int value: [%d]", nBufSize);
        return ret;
    }
    
    ret.resize(static_cast<size_t>(processCount));
    
    proc_listpids(PROC_ALL_PIDS, 0, ret.data(), static_cast<int>(nBufSize));
    return ret;
}

bool ProcessWrapper::getProcessInfo(pid_t pid, proc_bsdinfo* pProcInfo)
{
    if (pProcInfo == nullptr)
    {
        return false;
    }
    return (PROC_PIDTBSDINFO_SIZE == proc_pidinfo(pid, PROC_PIDTBSDINFO, 0, pProcInfo, PROC_PIDTBSDINFO_SIZE));
}
