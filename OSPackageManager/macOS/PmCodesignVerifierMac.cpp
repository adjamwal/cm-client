...
static int
check_signature( const char* path)
{
    int rc = -1;
...
static int
check_signer( const char* path, const char* signer, SigType sig_type, uint64_t killdate )
{
    int rc = -1;
...
CodeSignStatus CodesignVerifier::VerifyWithKilldate( const std::string& strPath, const std::string& strSigner, SigType sig_type, uint64_t killdate )
{
    CodeSignStatus status = CodeSignStatus::CODE_SIGN_VERIFICATION_FAILED;
    if (strPath.empty() || strSigner.empty())
    {
        PM_LOG_DEBUG("Invalid Input" );
        return status;
    }
    PM_LOG_DEBUG("verifying file signature: file = [%s], signer = [%s], type = [%d]", strPath.c_str(), strSigner.c_str(), sig_type );
    int iRet = check_signature(strPath.c_str());
    if (iRet >= 0)
    {
        iRet = check_signer(strPath.c_str(), strSigner.c_str(), sig_type, killdate);
        if (iRet >= 0)
        {
...
CodeSignStatus CodesignVerifier::Verify( const std::string& strPath, const std::string& strSigner, SigType sig_type )
{
    if((strPath.empty()) || (strSigner.empty()) || (SigType::SIGTYPE_NATIVE != sig_type))
    {
        PM_LOG_ERROR("invalid parameters [%ls] : [%ls]", strPath.c_str(), strSigner.c_str());
        return CodeSignStatus::CODE_SIGN_FAIL;
    }
    return VerifyWithKilldate( strPath.c_str(), strSigner.c_str(), sig_type, KILLDATE );
}
